...
if(year != ""):
	...
	try: 
		...
		arcpy.AddError("Data year is undefined.")
		sys.exit(-1)
...
for dataset in boundariesSeq:
	...
	arcpy.SetParameterAsText(i, os.path.join(zipOutputFolder, dataset + ".shp"))
	i += 1
...
for dataset in dataSeq:
	...
	try:
		...
		arcpy.SetParameterAsText(i, saveFile.replace(".json", ".csv"))
		i += 1
	except:
